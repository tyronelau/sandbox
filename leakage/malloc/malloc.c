...
/* pagesize */
static size_t _dl_pagesize = 4096;
# define GLRO(x) _##x

static void __set_errno(int no) {
  errno = no;
}
...
{
  char buf[512];
  (void) simple_snprintf(buf, 512, "%s%s%s:%d: %s%sAssertion `%s' failed.\n",
...
void __libc_message(int do_abort, const char *fmt, ...) {
  /* char buf[1024];
  simple_snprintf(buf, 1024, fmt */
  char buf[1024];
  simple_snprintf(buf, 1024, "Error: %s", fmt);
